...
class TiposScreen extends StatefulWidget {
  ...
  TiposScreen({required this.tipo, required this.categoria});

  @override
  _TiposScreenState createState() => _TiposScreenState();
}
...
class _TiposScreenState extends State<TiposScreen> {
  ...
  void initState() {
    super.initState();
    _fetchProductsByTipo();

    print("wwwwwwwwwwwwwwwwwww__TiposScreenState");
    _products = apiService.getProducts();
  }

  Future<void> _fetchProductsByTipo() async {
    final fetchedProductsByTipo = await apiService.getProductsByTipo(widget.tipo, widget.categoria);
    setState(() {
      products = fetchedProductsByTipo;
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('La tienda -- Lista de Productos por categoria y tipo'),
      ),
